...
export default function CvRedactorPage() {
  ...
  const handleEyeDropper = async () => {
    if (!("EyeDropper" in window)) {
      alert("Trình duyệt không hỗ trợ hút màu (Dùng Chrome/Edge trên máy tính)");
      return;
    }
    try {
      // @ts-ignore
  ...
  const fetchPdfInfo = async (f: File) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/v1/cv/info`, { method: "POST", body: formData });
      if(res.ok) {
          const data = await res.json();
          const pages = data.total_pages || 1;
          setTotalPages(pages);
      }
    } catch (e) { console.error("Lỗi lấy info:", e); }
  };
  ...
  const fetchAllPreviews = async (f: File, total: number) => {
    ...
    for (let i = 0; i < total; i++) {
        ...
        try {
            const res = await fetch(`${API_BASE}/api/v1/cv/preview`, { method: "POST", body: formData });
            if (res.ok) {
                const blob = await res.blob();
                urls[i] = window.URL.createObjectURL(blob);
            }
        } catch (e) { console.error(`Lỗi tải trang ${i}:`, e); }
    }
    setPreviewUrls(urls);
    setLoading(false);
  };
  ...
  const handleMouseDown = (e: React.MouseEvent, pageIndex: number) => {
    ...
    setStartPos(pos);
    setCurrentBox({ id: Date.now(), x: pos.x, y: pos.y, w: 0, h: 0, pageIndex });
  };
  ...
  const handleProcess = async () => {
    ...
    if (activeTab === "manual") {
      ...
      const firstImage = document.getElementById("page-image-0") as HTMLImageElement;
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-20 shadow-sm">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="bg-slate-100 p-1 rounded-lg hidden md:flex">
             <button 
                onClick={() => { setActiveTab("auto"); setResultPdfUrl(null); }} 
                className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${activeTab === "auto" ? "bg-white text-blue-600 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
             >
               🤖 Tự động
             </button>
             <button 
                onClick={switchToManual} 
                className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${activeTab === "manual" ? "bg-white text-blue-600 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
             >
               🖐️ Thủ công
             </button>
          </div>
        ...
        {!file && (
           <div className="flex flex-col items-center justify-center mt-20 animate-in fade-in slide-in-from-bottom-4">
             ...
             >
               ...
               <p className="font-medium text-lg text-slate-600">Click để tải lên CV PDF</p>
               <p className="text-sm text-slate-400 mt-2">Hỗ trợ file PDF mọi định dạng</p>
             </div>
           </div>
        )}
          ...
          <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 mb-6 flex flex-wrap gap-4 items-center justify-between sticky top-20 z-10">
            <div className="flex items-center gap-3">
               ...
               <div>
                 ...
                 <button onClick={() => setFile(null)} className="text-xs text-red-500 hover:underline">Xóa / Chọn lại</button>
               </div>
            </div>
            ...
            <div className="flex gap-3">
               ...
               >
                 {loading ? (
                    ...
                    <>{activeTab === "auto" ? "🚀 Bắt đầu Che (Auto)" : "🔥 Xóa vùng đã chọn"}</>
                 )}
               </button>
            </div>
          </div>
                ...
                <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200 sticky top-40 space-y-6">
                    <div>
                        ...
                        <div className="flex gap-2 mb-2">
                            <button onClick={handleEyeDropper} className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 py-2 rounded-md text-sm font-medium flex items-center justify-center gap-2 border border-slate-300">
                                🖊️ Hút màu
                            </button>
                        </div>
                        <div className="flex items-center gap-2 border rounded p-1 bg-slate-50">
                            ...
                            <span className="text-sm text-slate-600 font-mono uppercase">{pickedColor}</span>
                        </div>
                    </div>
                    ...
                    <div className="pt-4 border-t">
                        <button onClick={() => setBoxes([])} className="w-full text-red-600 bg-red-50 hover:bg-red-100 py-2 rounded text-sm font-medium">
                            🗑️ Xóa hết vùng vẽ
                        </button>
                    </div>
                </div>
              ...
              <div className="lg:col-span-3 bg-slate-200 rounded-xl p-6 min-h-[500px] flex flex-col items-center gap-8 overflow-y-auto max-h-[85vh] border-2 border-dashed border-slate-300 scroll-smooth">
                ...
                {Array.from({ length: totalPages }).map((_, pageIndex) => (
                    previewUrls[pageIndex] ? (
                        <div key={pageIndex} className="relative shadow-2xl bg-white group/page transition-transform ">
                            ...
                            >
                                ...
                                {boxes.filter(b => b.pageIndex === pageIndex).map((box) => (
                                    <div key={box.id} style={{ position: "absolute", left: box.x, top: box.y, width: box.w, height: box.h, backgroundColor: pickedColor, opacity: 0.7, border: "1px solid rgba(0,0,0,0.2)" }} className="group">
                                        <button onClick={(e) => { e.stopPropagation(); removeBox(box.id); }} className="absolute -top-3 -right-3 bg-red-500 text-white rounded-full w-5 h-5 text-xs flex items-center justify-center opacity-0 group-hover:opacity-100 shadow-sm transition-opacity">×</button>
                                    </div>
                                ))}
                        ...
                        <div key={pageIndex} className="w-[800px] h-[1000px] bg-white animate-pulse rounded flex items-center justify-center text-slate-300">
           ...
           <div className="animate-in fade-in slide-in-from-bottom-4">
             ...
             <div className="bg-green-50 border border-green-200 rounded-xl p-4 mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
                <div className="flex items-center gap-3">
                   ...
                   <div>
                     <h3 className="font-bold text-green-800">Xử lý thành công!</h3>
                     <p className="text-sm text-green-600">Đã áp dụng che thông tin.</p>
                   </div>
                </div>
             ...
             {activeTab === "auto" ? (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-[85vh]">
                   <div className="bg-white rounded-xl shadow border overflow-hidden flex flex-col">
                      <div className="bg-slate-50 p-3 border-b text-xs font-bold text-slate-500 uppercase">Bản gốc</div>
                      {originalPdfUrl && <iframe src={`${originalPdfUrl}#toolbar=0&navpanes=0`} className="w-full h-full" />}
                   </div>
                   <div className="bg-white rounded-xl shadow border overflow-hidden flex flex-col">
                      <div className="bg-blue-50 p-3 border-b text-xs font-bold text-blue-600 uppercase">Kết quả Tự động quét (trang 1)</div>
                      <iframe src={`${resultPdfUrl}#toolbar=0&navpanes=0`} className="w-full h-full" />
                   </div>
                </div>
             ) : (
